...
void CameraTest::setProjectMatrix()
{
...
void CameraTest::move( float xPos, float zPos)
{
...
void CameraTest::fly( float yPos /* = 0.0f */)
{
	m_position.y -= yPos;
	m_look.y -= yPos;
	D3DXMATRIX viewMateral;
...
void CameraTest::rotateX( float x)
{
...
void CameraTest::rotateZ( float z)
{
	D3DXMATRIX outMateral;
...
void CameraTest::rotate(float x, float z)
{
	D3DXMATRIX outMateral;
	float angleX = x/CAMERA_ROTATE_DISTANCE * D3DX_PI;
	float angleZ = z/CAMERA_ROTATE_DISTANCE * D3DX_PI;
